package com.evgeniy.test.behancer.data;

import android.support.v4.util.Pair;

import com.evgeniy.test.behancer.data.database.BehanceDao;
...
public class Storage {
    ...
    public ProjectResponse getProjectByName(String username){
        List<Project> projects = mBehanceDao.getProjectsByName(username);
        for (Project project : projects) {
            project.setCover(mBehanceDao.getCoverFromProject(project.getId()));
            project.setOwners(mBehanceDao.getOwnersFromProject(project.getId()));
        }
